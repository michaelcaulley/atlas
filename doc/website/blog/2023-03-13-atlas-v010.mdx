...
It's been two months since v0.9.0, and it's time to release a new version and share with you what we've accomplished so
far, as well as what's to come in the upcoming weeks. Besides the many improvements and bug fixes in v0.10.0, we
added two major integrations to Atlas that I want to share with you: schema loaders and cloud-based migration linting.

## Schema Loaders

In our previous post, we discussed our motivation for developing an infrastructure to load desired states from external
sources (not just SQL and HCL), and we highlighted the importance of schema loaders. I'm happy to share that we've made
significant progress on this front! We started by integrating with the [Ent framework](https://entgo.io), and with the
...
I'm personally very excited about this integration because it has proven to work well for the Ent community, and this
is just the beginning. We are ready to expand support for additional frameworks and languages. In the upcoming weeks,
you can expect to see additional integrations, such as GORM, Sequelize, and more. With these superpowers of Atlas,
users will be able to manage their entire projects using a single tool - Atlas!

## Migration Directory Linting

Another exciting capability that was added to Atlas - the ability to connect remote migration directories stored in GitHub
...
1\. Login to [atlasgo.cloud](https://auth.atlasgo.cloud/login) and create yourself a new workspace (organization) for
your projects:
...
4\. Now, every new change made to the migration directory will be reflected in Atlas Cloud. But what about the changes
themselves? Here's where the magic happens. Once the directory is connected, any pull request that modifies this
directory will be checked and reviewed by Atlas!

Let's create a sample migration change, open a pull request, and see what it looks like in action:
...
and much more. If any of these features sound interesting to you, please do not hesitate to contact us.
